...
/// Verifies that ``NavProcessorAgentToInternalAdapter`` correctly bridges a
/// ``NavigationModuleEventProcessor`` (agent facade) into the internal
/// ``NavigationEventProcessor`` protocol used by the navigation engine.
final class NavProcessorAgentToInternalAdapterTests: XCTestCase {

    // MARK: - Passthrough

    func testPassthroughName() {
        let adapter = NavProcessorAgentToInternalAdapter(wrapping: PassthroughModuleProcessor())
    ...
    func testSuppression() {
        let adapter = NavProcessorAgentToInternalAdapter(wrapping: SuppressingModuleProcessor())
    ...
    func testStringAttributes() {
        ...
        let adapter = NavProcessorAgentToInternalAdapter(wrapping: AttributeModuleProcessor(attributes: attributes))
    ...
    func testNilAttributes() {
        let adapter = NavProcessorAgentToInternalAdapter(wrapping: AttributeModuleProcessor(attributes: nil))
    ...
    func testTypeNameForwarded() {
        ...
        let adapter = NavProcessorAgentToInternalAdapter(wrapping: processor)
