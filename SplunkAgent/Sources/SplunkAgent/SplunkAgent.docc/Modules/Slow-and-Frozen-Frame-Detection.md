...
Detected frames are periodically flushed as aggregated counts, not one span per frame. Each flush emits `slowRenders` and `frozenRenders` spans carrying a `count` attribute.
