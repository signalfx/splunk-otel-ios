...
FRAMEWORK_ANALYTICS="SplunkOtel"
...
function zipped_xcframework_filename {
    ...
    if [[ $framework == $FRAMEWORK_ANALYTICS ]]; then
        echo "SplunkOtel-iOS-$(artifact_build_number $framework).zip"
    fi
}
...
function zipped_xcframework_url {
    ...
    echo "https://appdynamics.jfrog.io/ui/native/zip-hosted/mrum/SplunkOtel-iOS/${artifact_version}/${file_name}"
}
