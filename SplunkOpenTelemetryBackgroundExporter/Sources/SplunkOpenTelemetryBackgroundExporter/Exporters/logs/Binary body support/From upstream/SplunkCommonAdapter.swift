...
enum SplunkCommonAdapter {
    static func toProtoAttribute(
        key: String,
        attributeValue: SplunkAttributeValue
    )
        -> Opentelemetry_Proto_Common_V1_KeyValue
    {
        ...
        case let .set(value):
            keyValue.value.kvlistValue.values = value.labels.map {
                toProtoAttribute(key: $0, attributeValue: SplunkAttributeValue(otelAttributeValue: $1))
            }

        case let .array(value):
            keyValue.value.arrayValue.values = value.values.map {
                toProtoAnyValue(attributeValue: SplunkAttributeValue(otelAttributeValue: $0))
            }

        case let .stringArray(value):
            keyValue.value.arrayValue.values = value.map {
                toProtoAnyValue(attributeValue: .string($0))
            }

        case let .boolArray(value):
            keyValue.value.arrayValue.values = value.map {
                toProtoAnyValue(attributeValue: .bool($0))
            }

        case let .intArray(value):
            keyValue.value.arrayValue.values = value.map {
                toProtoAnyValue(attributeValue: .int($0))
            }

        case let .doubleArray(value):
            keyValue.value.arrayValue.values = value.map {
                toProtoAnyValue(attributeValue: .double($0))
            }
        }
    ...
    static func toProtoInstrumentationScope(
