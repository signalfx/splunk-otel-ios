...
func lifecycleEvent(_ event: String) {
    ...
    if event == UI_APPLICATION_WILL_RESIGN_ACTIVE_NOTIFICATION ||
        ...
        if activeSpan == nil {
            let span = buildTracer().spanBuilder(spanName: event == UI_APPLICATION_WILL_RESIGN_ACTIVE_NOTIFICATION ? Attribute.SpanName.SPAN_NAME_APP_RESIGNACTIVE : Attribute.SpanName.SPAN_NAME_APP_ENTERFOREGROUND).startSpan()
            span.setAttribute(key: Attribute.KeyValue.COMPONENT_KEY, value: Attribute.Component.COMPONENT_APP_LIFECYCLE)
    ...
    if event == UI_APPLICATION_WILL_TERMINATE_NOTIFICATION {
        let now = Date()
        let span = buildTracer().spanBuilder(spanName: Attribute.SpanName.SPAN_NAME_APP_TERMINATING).setStartTime(time: now).startSpan()
        span.setAttribute(key: Attribute.KeyValue.COMPONENT_KEY, value: "AppLifecycle")
        span.end(time: now)
    }
