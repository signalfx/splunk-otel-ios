...
class SlowFrameBeatingHeartView: UIView {
    ...
    func startAnimation() {
        // Even with 0.04 it's still not the smoothest but it serves the purpose
    ...
    private func updateAnimation() {
        ...
        // Use a manually conducted animation so that we can keep it on the main thread
        // and witness the results of the main thread stall in the UI.
        //
        // If we instead used a built-in animation here, Apple would put it on
        // a background worker thread and it would not (reliably) pause
        // when the main thread pauses, such as during a sleep on the main thread,
        // so the results would not be visible or clearly shown in this test app.
