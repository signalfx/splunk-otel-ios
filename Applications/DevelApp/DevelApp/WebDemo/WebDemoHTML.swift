    ...
    public static func loadWebViewContentWithBRUM(for webView: WKWebView, scriptContent: String) {
        let initialContent = """
            ...
            <body style="font-size: 48px; font-family: sans-serif">
                ...
                <h4>Current Native Session ID:</h4>
                <p id="sessionId">unavailable</p>
                <p>Above session ID is not from BRUM's detection. To see whether BRUM picked up the session ID,
                watch the traffic over the wire as reported in the Xcode Console while tethered to Xcode
                (such as when running in the iOS Simulator). You will need to have set your token and realm
                in the brumScript() function in WebDemoJS.swift.
            </body>
            </html>
            """
        webView.loadHTMLString(initialContent, baseURL: nil)
